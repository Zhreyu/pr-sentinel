...
export interface ExtendedIntentClassification extends IntentClassification {
  maintainerSummary?: string;
  overallConfidence?: number;
  needsMaintainerAttention?: boolean;
  contextNotes?: string[];
  topFilesToInspect?: TopFile[];
}

export interface ExtendedReviewSuggestion extends ReviewSuggestion {
  whyItMatters?: string;
}
...
export const prAnalyses = pgTable(
  "pr_analyses",
  {
    ...
    // Analysis results
    intentClassification: jsonb("intent_classification").notNull().$type<ExtendedIntentClassification>(),
    ...
    reviewSuggestions: jsonb("review_suggestions").default([]).$type<ExtendedReviewSuggestion[]>(),
