...
import { repositories, pullRequests, organizations, organizationMembers, users } from "@pr-sentinel/database/schema";
...
async function handleInstallation(payload: InstallationPayload): Promise<NextResponse> {
  const { action, installation, repositories: repos, sender } = payload;

  console.log("Installation event", { action, installationId: installation.id, sender: sender?.login });
  ...
  if (action === "created") {
    ...
    // Link the installing user (sender) as org owner
    if (sender?.id) {
      const installingUser = await db
        .select()
        .from(users)
        .where(eq(users.githubUserId, sender.id))
        .limit(1)
        .then((rows) => rows[0]);
      ...
      if (installingUser) {
        ...
        const existingMember = await db
          ...
          .where(
            and(
              ...
              eq(organizationMembers.userId, installingUser.id)
            )
          )
          .limit(1)
          .then((rows) => rows[0]);
        ...
        if (!existingMember) {
          await db.insert(organizationMembers).values({
            ...
            userId: installingUser.id,
            role: "owner",
          });
          console.log("Added org owner", { userId: installingUser.id, orgId: org.id });
        }
      } else {
        console.warn("Installing user not found in DB, they may need to log in first", { senderGithubId: sender.id });
      }
    }
      ...
      if (!existingPR) {
        ...
        const newPR = await db
          ...
          .values({
            ...
            authorAssociation: fullPR.author_association ?? null,
            isFromFork: Boolean(fullPR.head.repo?.fork),
            commitCount: fullPR.commits ?? 0,
      ...
      if (existingPR) {
        await db
          ...
          .set({
            ...
            commitCount: pr.commitCount ?? existingPR.commitCount,
            headSha: pr.headSha,
            baseSha: pr.baseSha,
